...
class C2VDataset(Dataset):
    ...
    def __init__(self, cell_tgt_file: str, valid_node_file: str, cell_sim_file:str):
        ...
        self.cell_sim = torch.from_numpy(np.load(cell_sim_file)).float()

    def __len__(self):
        return self.tgt.shape[0]

    def __getitem__(self, item):
        return torch.tensor(item, dtype=torch.long), self.tgt[item], self.cell_sim[item]
